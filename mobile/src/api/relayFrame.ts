...
//
// The two implementations have to agree byte for byte or this app joins a group
// and can speak to nobody in it, so everything that could drift is stated in one
// place here: the domain string (seedphrase.ts), the additional-data layout
// below, the nonce length, and the nonce||ciphertext framing. The shared vector
// is internal/relay/announce_seal_test.go, which extension/check-relay-seal.mjs
// holds all three ports to.
//
// The cipher comes from a library although sha256.ts next door is
// hand-written. AES-GCM fails silently and totally: a repeated nonce does not
// corrupt one message, it hands over the authentication key. @noble/ciphers is
// audited, dependency-free and pure JavaScript, so it adds no native surface.
//
// expo-crypto is native and is here for one call, real randomness for the
// nonce. React Native ships no crypto.getRandomValues, and Math.random is
// seeded, predictable and repeats, which is the one thing AES-GCM cannot
// survive.
...
/**
 * Mirrors relay.Identity: the part of an announce the relay never reads. The
 * split follows the fields the relay server touches, which is the instance id
 * and nothing else.
 */
...
const utf8 = (s: string): Uint8Array => {
  // Written out rather than TextEncoder, which seedphrase.ts also does, so the
  // two agree and neither depends on what a runtime happens to provide.
  const out: number[] = [];
  for (const ch of s) {
 ...
 * protocol.go: the routing fields that travel in the clear, bound into the
 * seal so a relay cannot redirect a frame and have it open somewhere else. The
 * \x00 separator and the per-direction label both matter; the label is what
 * stops an answer being replayed as a question.
 */
 ...
 * framing relay.seal writes: the Go side reads the first NONCE_LEN bytes as
 * the nonce and hands the rest to GCM.
 */
...
/**
 * open reverses seal. A wrong key, a truncated frame and a tampered one all
 * return null, as relay.ErrSealed does: the caller's move is the same in every
 * case, and telling them apart tells an attacker which guess was closer.
 */
 ...
 *  relay.SealIdentity.
 *
 *  The phone's announce carries little: the constant 'mobile' and a name the
 *  person chose. It is sealed anyway, because a relay that can tell a phone's
 *  announce from an instance's by shape alone learns which member of a group is
 *  the phone, which is a fact about a household rather than about a protocol. */
 ...
 *  Returns null for a blob that does not open, which the caller reads as "list
 *  this peer under its id, unnamed" rather than "this peer is not there". */
