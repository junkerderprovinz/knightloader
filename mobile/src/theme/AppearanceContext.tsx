...
interface Override {
  ...
  rainbow?: boolean;
  /**
   * WHICH slot is chosen, as its own fact.
   *
   * It used to be derived from the accent by finding the nearest preset, which
   * works only while every swatch holds a different colour. Mix two of them to
   * the same red - which is precisely what jdp did on 2026-09-02 - and the
   * arithmetic marks both, so the row shows two chosen swatches and a press on
   * either opens the picker instead of choosing. A choice is not recoverable
   * from a value once two values are equal, so it is stored.
   */
  slot?: number;
  /**
...
export function AppearanceProvider({ children }: { children: ReactNode }) {
  ...
  /**
   * The live override, mirrored into a ref so a writer never has to wait for a
   * render to see what the last writer did.
   *
   * This is the fix for "nicht alle farbfelder speichern dann die farbe" (jdp,
   * 2026-09-02). Every setter below used to spread the `override` its closure
   * was built with, and two writes in quick succession - mixing one swatch, then
   * the next - are exactly that: the second one still saw the state from before
   * the first, spread it, and wrote the first change back out of existence.
   * Nothing about it looked wrong on screen until you went back to the earlier
   * swatch and found the preset there.
   */
   ...
   * now - which is what anything spreading the previous value must do.
   */
