import type { Dict } from './en';

export const es: Dict = {

  'qr.cancel': 'Cancelar',
  'qr.cameraPermissionHint': 'Se necesita acceso a la cámara para escanear el código QR.',
  'qr.grantAccess': 'Permitir acceso',

  'connections.addButton': '+ Conexión',
  'connections.empty': 'Todavía no hay ninguna conexión guardada.',
  'connections.emptyButton': 'Añade tu primera conexión',
  'connections.remove': 'Eliminar',


  'downloads.connected': 'conectado',
  'downloads.connecting': 'conectando…',
  'downloads.overviewLink': 'Resumen',
  'downloads.queueHalted': 'Detenida',
  'downloads.queueRunning': 'En marcha',
  'downloads.queueActive': '{n} activas',
  'downloads.empty': 'No hay descargas.',
  'downloads.emptyConnecting': 'Conectando con el servidor…',
  'overview.title': 'Todas las instancias',
  'overview.online': '{n}/{total} en línea',
  'instance.files': 'archivos',
  'instance.left': 'restantes',
  'instance.offline': 'sin conexión',
  'instance.checking': 'Comprobando',
  'instance.online': 'En línea',
  'instance.captchas': '{n} captchas esperando ahora mismo',
  'instance.captchasOne': '1 captcha esperando ahora mismo',
  'downloads.start': 'Iniciar',
  'downloads.stop': 'Detener',
  'downloads.startHeld': '{n} enlace(s) se han rechazado y no se han iniciado.',
  'downloads.startBlocked': 'Una programación mantiene la cola en pausa.',
  'downloads.tabDownloads': 'Descargas',
  'downloads.tabCollector': 'Colector',
  'packages.loose': 'Sin paquete',
  'packages.start': 'Iniciar este paquete',
  'packages.expand': 'Abrir este paquete',
  'packages.collapse': 'Cerrar este paquete',
  'packages.delete': 'Quitar este paquete',
  'packages.deleteConfirmTitle': '¿Quitar el paquete?',
  'packages.deleteConfirmMessage': '{n} enlaces salen de la lista. Los archivos ya descargados se mantienen.',
  'packages.deleteConfirmButton': 'Quitar',

  'captcha.screenTitle': 'Captchas',
  'captcha.title': 'Captcha necesario',
  'captcha.titleMore': 'Captcha necesario ({n} más en espera)',
  'captcha.open': 'Responder',
  'captcha.waiting': 'Un captcha de {host} está esperando respuesta.',
  'captcha.empty': 'No hay ningún captcha esperando.',
  'captcha.answerLabel': 'Respuesta',
  'captcha.answerPlaceholder': 'Escribe lo que ves…',
  'captcha.clickHint': 'Toca cada punto que pide la imagen y luego pulsa Continuar.',
  'captcha.clickCount': '{n} punto(s) marcado(s)',
  'captcha.clickClear': 'Borrar puntos',
  'captcha.widgetHint': 'Resuelve el desafío de abajo. Todo continúa solo en cuanto lo hagas.',
  'captcha.widgetScoreHint': 'No tienes que tocar nada. El servicio de captcha comprueba este teléfono y entrega su respuesta por sí solo.',
  'captcha.solve': 'Resolver',
  'captcha.widgetUnavailable': 'Este reto no se pudo cargar.',
  'captcha.widgetUnreachable': 'El teléfono no pudo contactar con la instancia ni con el servicio de captcha. Puede que un filtro de red lo esté bloqueando. Actualizar lo vuelve a intentar.',
  'captcha.widgetRefused':
    'El servicio de captcha se negó a funcionar aquí e informó de {code}. Muchos hosters atan su captcha a su propia web, así que puede que no cargue en absoluto en KnightLoader. Actualizar lo vuelve a intentar, y Cancelar se salta este captcha.',
  'captcha.widgetPageRefused': 'La instancia se negó a cargar este reto (error {code}). Cancelar se salta este captcha.',
  'captcha.widgetRelayTitle': 'Resuelve este en la interfaz web',
  'captcha.widgetRelayReason': 'En una conexión hecha con la frase, la aplicación solo responde captchas de imagen y de clic. Responde este en la interfaz web o cancélalo aquí.',
  'captcha.unsupported': 'KnightLoader no puede mostrar este tipo de reto (indicado como {vendor}).',
  'captcha.unsupportedHint': 'Usa Cancelar abajo, o bloquea los captchas de este hoster para esta sesión.',
  'captcha.unsolvable': 'KnightLoader no puede resolver este captcha.',
  'captcha.unsolvableVendor': 'JDownloader no dijo de qué servicio de captcha se trata, y KnightLoader no adivina. Cancelar se salta este captcha.',
  'captcha.unsolvableAction': 'Es una comprobación de reCAPTCHA v3, y JDownloader no pasó la acción que pide el hoster. Una respuesta sin ella se rechazaría. Cancelar se salta este captcha.',
  'captcha.unsolvableTurnstile': 'KnightLoader no puede mostrar por sí mismo un reto de Cloudflare Turnstile, así que solo puede responderlo un solucionador de pago configurado en los ajustes de Captcha de la instancia. Cancelar se salta este captcha.',
  'captcha.continue': 'Continuar',
  'captcha.cancel': 'Cancelar',
  'captcha.refresh': 'Actualizar',
  'captcha.close': 'Cerrar',
  'captcha.moreOptions': 'Más opciones',
  'captcha.blockHoster': 'Dejar también de preguntar por {host} esta sesión',
  'captcha.blockEverywhere': 'Dejar también de preguntar por cualquier hoster esta sesión',
  'captcha.tooLate': 'Esa respuesta llegó demasiado tarde.',
  'captcha.timedOut': 'Un captcha de {host} caducó.',
  'captcha.resolvedElsewhere': 'Un captcha de {host} se resolvió en otro sitio.',
  'captcha.relayRefused': 'Esta instancia es demasiado antigua para pasar captchas a la aplicación. Actualiza KnightLoader en ella.',
  'captcha.solverWaiting': 'Un solucionador toma el relevo en {time}.',
  'captcha.solverWaitingHint': 'Estás mirando, así que los solucionadores esperan primero tu respuesta. En cuanto la aplicación pase a segundo plano o se acabe el tiempo, toma el relevo el primer solucionador según tu orden.',
  'captcha.solverSolving': '{solver} está resolviendo este captcha.',
  'captcha.solverSolvingHint': 'Aun así puedes responderlo tú. Vale la respuesta que llegue primero.',
  'captcha.solverStopped': 'Ningún solucionador pudo encargarse de este captcha.',
  'captcha.solverStoppedTaken': '{solver} se encargó de este captcha, pero no envió ninguna respuesta.',
  'captcha.solverNotPassedOn': 'Puede que {solver} lo cobre igualmente, así que KnightLoader no lo envía además a otro solucionador. Todavía puedes responderlo tú.',
  'captcha.solverUnsupported': '{solver} no resuelve este tipo de captcha.',
  'captcha.solverNoAnswer': '{solver} se encargó de él, pero no llegó ninguna respuesta.',
  'captcha.solverGaveUp': '{solver} se encargó de él y luego se rindió: {reason}',
  'captcha.solverFailed': 'No se pudo contactar con {solver}.',
  'captcha.solverRefused': '{solver} lo rechazó: {reason}',

  'addDownload.title': 'Añadir enlaces',
  'addDownload.titlePeer': 'Añadir enlaces a {name}',
  'addDownload.hint': 'Un enlace por línea, igual que en el cuadro de pegado de la interfaz web.',
  'addDownload.placeholder': 'https://…',
  'addDownload.errorEmpty': 'Pega al menos un enlace.',
  'addDownload.errorServer': 'Servidor: {message}',
  'addDownload.errorGeneric': 'No se pudieron enviar los enlaces.',
  'error.federationOff': 'Las instancias asociadas están desactivadas en la instancia a la que estás conectado, así que no puede llegar a las demás.',
  'addDownload.cancel': 'Cancelar',
  'addDownload.button': 'Añadir',

  'status.queued': 'en cola',
  'status.running': 'en curso',
  'status.paused': 'pausado',
  'status.finished': 'terminado',
  'status.failed': 'fallido',
  'status.extracting': 'extrayendo',

  'settings.title': 'Ajustes',
  'settings.back': 'Atrás',
  'settings.language': 'Idioma',
  'settings.appearance': 'Apariencia',
  'settings.appearanceFollows': 'El aspecto de la aplicación sigue la instancia predeterminada',
  'settings.appearanceOverridden': 'Definida aquí, así que ya no sigue a la instancia.',
  'settings.followInstance': 'Seguir la instancia predeterminada',
  'settings.theme': 'Tema',
  'settings.theme.light': 'Claro',
  'settings.theme.dark': 'Oscuro',
  'settings.corners': 'Esquinas',
  'settings.corners.round': 'Redondeadas',
  'settings.corners.slightlyRounded': 'Suaves',
  'settings.corners.square': 'Rectas',
  'settings.corners.leaf': 'Hoja',
  'settings.accent': 'Acento',
  'settings.rainbow': 'Modo arcoíris',
  'settings.disco': 'Disco',
  'settings.discoGlideHint': 'Mientras el modo arcoíris está activado, todos los elementos de color se deslizan juntos por la paleta, un color cada 2,4 segundos. Si «Movimiento» está en «Desactivado», o si tienes activada la reducción de movimiento en los ajustes de tu teléfono, en su lugar salta de un color al siguiente.',
  'settings.problems': '¿Problemas?',
  'settings.problemsHint': 'Envía esto cuando informes de algo. Ahorra una ronda de preguntas y no lleva ni dirección ni token.',
  'settings.problemsCopy': 'Copiar informe',
  'settings.problemsCopied': 'Informe copiado',
  'settings.about': 'Acerca de',
  'settings.version': 'Versión {version}',
  'settings.githubLink': 'Ver en GitHub',
  'task.mode.free': 'Gratis',
  'task.mode.premium': 'Premium',
  'task.remote': 'Descargando en {service}',
  'settings.aboutTitle': 'Acerca de KnightLoader',
  'settings.aboutBody': 'Un solo caballero, una cruzada: herramientas de código abierto bien hechas que no existían con la forma que yo quería, construidas hasta que no falte nada. Todo gratis, sin cuentas, sin telemetría, sin anuncios y sin planes de pago. Ningún asterisco en ninguna parte. Nada legible sale nunca de tus propios muros. Forjado en tardes y fines de semana, con corazón y terquedad.',
  'settings.accentReset': 'Predeterminado',
  'settings.aboutCoffeeButton': 'Invítame a un café',
  'settings.aboutPaypal': 'PayPal',
  'settings.aboutCrypto': 'Cripto',
  'settings.cryptoTitle': 'Donar con cripto',
  'settings.donateAppeal': 'Echa una moneda a tu caballero: cubre parte de los costes y mantiene vivo el proyecto. Todo lo que hago es gratis y sin anuncios.',
  'settings.cryptoIntro': 'Elige una criptomoneda y una red, luego escanea el código o copia la dirección.',
  'settings.cryptoNetworks': 'Redes',
  'settings.cryptoNoTag': 'No hacen falta destination tag ni memo.',
  'settings.cryptoCopy': 'Copiar',
  'settings.cryptoCopied': 'Copiado',
  'settings.cryptoClose': 'Cerrar',
  'settings.aboutCoffee': 'Si KnightLoader se ha ganado un sitio en tu servidor o tu ordenador, echa una moneda a tu caballero: ayuda a cubrir los costes y mantiene vivo el proyecto. Y hace que el corazón de este caballero lata un poco más rápido. Abajo hay tres caminos, elige el que te venga bien.',
  'settings.aboutReport': '¿Problemas, deseos o sugerencias? Abre una incidencia en GitHub, o envía un correo.',
  'settings.aboutVersion': 'Versión',
  'settings.aboutGithub': 'GitHub',
  'settings.aboutMail': 'Correo',
  'settings.aboutMailSubject': 'Comentarios',
  'settings.dangerZone': 'Zona de peligro',
  'settings.removeAllConnections': 'Eliminar todas las conexiones',
  'settings.removeAllConfirmTitle': '¿Eliminar todas las conexiones?',
  'settings.removeAllConfirmMessage':
    'Elimina de este dispositivo cada conexión guardada junto con su token. En los servidores no cambia nada.',
  'settings.removeAllConfirmButton': 'Eliminar todo',
  'settings.cancel': 'Cancelar',
  'settings.rainbowPalettePosition': 'Posición {position} de la paleta',
  'settings.rainbowPalette': 'Paleta de colores',
  'settings.pickerDone': 'Hecho',

  'relay.title': 'Conectar con una frase',
  'relay.hint':
    'Lee las doce palabras en cualquier KnightLoader que ya tengas y escríbelas aquí. Cada instancia de ese grupo aparecerá abajo. No necesitas dirección, clave ni cuenta.',
  'relay.phraseLabel': 'Frase de conexión',
  'relay.phrasePlaceholder': 'las doce palabras de tu instancia',
  'relay.joinButton': 'Conectar',
  'relay.qrHintPhrase': 'Escanea el código QR que aparece junto a la frase',
  'relay.scanButton': 'Escanear el código QR',
  'relay.pasteButton': 'Pegar',
  'relay.saveAllButton': 'Añadir las {count}',
  'phrase.errChecksum': 'Esa frase no es válida. Comprueba si hay una palabra mal escrita o cambiada de sitio.',
  'phrase.errUnknownWord': 'La palabra {position} («{word}») no es una de las palabras admitidas.',
  'phrase.errWordCount': 'Son {count} palabras, pero una frase necesita {need}.',
  'relay.instancesTitle': 'En este relé',
  'relay.noInstances': 'Ahora mismo no hay ninguna instancia de este grupo en línea. Comprueba la frase y que al menos una instancia esté funcionando.',

  'settings.motion': 'Movimiento',
  'settings.motion.off': 'Desactivado',
  'settings.motion.subtle': 'Sutil',
  'settings.motion.wild': 'Intenso',
  'settings.motion.storm': 'Tormenta',
  'settings.accentRainbowOwns': 'El modo arcoíris reparte ahora los colores por posición. El color de acento sigue pintando todo lo que no tiene posición.',
  'settings.motionReduced': 'Tu teléfono está configurado para reducir el movimiento. Mientras sea así, aquí no se mueve nada, sea cual sea el nivel elegido.',
  'settings.rainbowPaletteNoInstance': 'La paleta pertenece a la instancia. Sin conexión no hay adónde escribir un color, así que las posiciones conservan los ocho colores integrados.',

  'settings.motionHint': 'Desactivado apaga toda animación no esencial; Sutil mantiene las entradas breves; Intenso es el que más movimiento tiene.',
};
