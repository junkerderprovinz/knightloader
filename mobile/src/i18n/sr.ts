import type { Dict } from './en';

export const sr: Dict = {

  'qr.cancel': 'Otkaži',
  'qr.cameraPermissionHint': 'Za skeniranje QR koda potreban je pristup kameri.',
  'qr.grantAccess': 'Dozvoli pristup',

  'connections.addButton': '+ Veza',
  'connections.empty': 'Još uvek nije sačuvana nijedna veza.',
  'connections.emptyButton': 'Dodajte svoju prvu vezu',
  'connections.remove': 'Ukloni',


  'downloads.connected': 'povezano',
  'downloads.connecting': 'povezivanje…',
  'downloads.overviewLink': 'Преглед',
  'downloads.queueHalted': 'Zaustavljeno',
  'downloads.queueRunning': 'U toku',
  'downloads.queueActive': '{n} aktivno',
  'downloads.empty': 'Nema preuzimanja.',
  'downloads.emptyConnecting': 'Povezivanje sa serverom…',
  'overview.title': 'Све инстанце',
  'overview.online': '{n}/{total} на мрежи',
  'instance.files': 'фајлова',
  'instance.left': 'преостало',
  'instance.offline': 'ван мреже',
  'instance.checking': 'Провера',
  'instance.online': 'На мрежи',
  'downloads.start': 'Покрени',
  'downloads.stop': 'Заустави',
  'downloads.startSkipped': 'Филтер веза задржава {n} веза.',
  'downloads.startBlocked': 'Распоред држи ред заустављеним.',
  'downloads.tabDownloads': 'Преузимања',
  'downloads.tabCollector': 'Сакупљач',
  'packages.loose': 'Без пакета',
  'packages.start': 'Покрени овај пакет',
  'packages.expand': 'Отвори овај пакет',
  'packages.collapse': 'Затвори овај пакет',
  'packages.delete': 'Уклони овај пакет',
  'packages.deleteConfirmTitle': 'Уклонити пакет?',
  'packages.deleteConfirmMessage': '{n} веза нестаје са списка. Већ преузете датотеке остају.',
  'packages.deleteConfirmButton': 'Уклони',

  'addDownload.title': 'Dodaj veze',
  'addDownload.titlePeer': 'Dodaj veze – {name}',
  'addDownload.hint': 'Jedna veza po redu, isto kao u polju za lepljenje u veb interfejsu.',
  'addDownload.placeholder': 'https://…',
  'addDownload.errorEmpty': 'Nalepite bar jednu vezu.',
  'addDownload.errorServer': 'Server: {message}',
  'addDownload.errorGeneric': 'Veze nije bilo moguće poslati.',
  'addDownload.cancel': 'Otkaži',
  'addDownload.button': 'Dodaj',

  'status.queued': 'na čekanju',
  'status.running': 'u toku',
  'status.paused': 'pauzirano',
  'status.finished': 'završeno',
  'status.failed': 'neuspelo',
  'status.extracting': 'raspakivanje',

  'settings.title': 'Podešavanja',
  'settings.back': 'Назад',
  'settings.language': 'Jezik',
  'settings.appearance': 'Izgled',
  'settings.appearanceFollows': 'Изглед апликације прати подразумевану инстанцу',
  'settings.appearanceOverridden': 'Podešeno ovde, pa više ne prati instancu.',
  'settings.followInstance': 'Прати подразумевану инстанцу',
  'settings.theme': 'Tema',
  'settings.theme.light': 'Svetla',
  'settings.theme.dark': 'Tamna',
  'settings.corners': 'Uglovi',
  'settings.corners.round': 'Okrugli',
  'settings.corners.soft': 'Blagi',
  'settings.corners.square': 'Oštri',
  'settings.accent': 'Naglasak',
  'settings.rainbow': 'Duga',
  'settings.problems': 'Problemi?',
  'settings.problemsHint': 'Pošaljite ovo kada nešto prijavljujete - uštedeće jedan krug pitanja. U njemu nema ni adrese ni tokena.',
  'settings.problemsCopy': 'Kopiraj izveštaj',
  'settings.problemsCopied': 'Izveštaj kopiran',
  'settings.about': 'O aplikaciji',
  'settings.version': 'Verzija {version}',
  'settings.githubLink': 'Pogledaj na GitHub-u',
  'task.mode.free': 'Бесплатно',
  'task.mode.premium': 'Премијум',
  'settings.aboutTitle': 'О KnightLoader-у',
  'settings.aboutBody': 'Самостално хостован менаџер преузимања с уредним сучељем, као алтернатива JDownloader-у. Мотор, API и сучеље стају у један програм, поред њега нема шта да се инсталира. Написано увече и викендом, јер је сваки други тражио два клика тамо где је био довољан један.',
  'settings.accentReset': 'Подразумевано',
  'settings.aboutCoffeeButton': 'Части ме кафом',
  'settings.aboutCoffee': 'Свиђа вам се KnightLoader? Кафа би ми улепшала дан.',
  'settings.aboutReport': 'Проблеми, жеље или предлози? Отвори проблем на GitHub-у или пошаљи е-пошту.',
  'settings.aboutVersion': 'Верзија',
  'settings.aboutGithub': 'GitHub',
  'settings.aboutMail': 'Пошаљи е-пошту',
  'settings.aboutMailSubject': 'Повратна информација',
  'settings.dangerZone': 'Opasna zona',
  'settings.removeAllConnections': 'Ukloni sve veze',
  'settings.removeAllConfirmTitle': 'Ukloniti sve veze?',
  'settings.removeAllConfirmMessage':
    'Ovo uklanja svaku sačuvanu vezu i njen token sa ovog uređaja. Na samim serverima se ništa ne menja.',
  'settings.removeAllConfirmButton': 'Ukloni sve',
  'settings.cancel': 'Otkaži',
  'settings.rainbowPalettePosition': 'Позиција палете {position}',
  'settings.rainbowPalette': 'Палета боја',
  'settings.pickerDone': 'Готово',

  'relay.title': 'Повежи се фразом',
  'relay.hint':
    'Очитај дванаест речи са било ког KnightLoader-а који већ користиш и упиши их овде. Свака инстанца те групе појавиће се испод — без адресе, без кључа, без налога.',
  'relay.phraseLabel': 'Фраза за повезивање',
  'relay.phrasePlaceholder': 'дванаест речи твоје инстанце',
  'relay.joinButton': 'Повежи',
  'relay.qrHintPhrase': 'Скенирај QR код приказан поред фразе',
  'relay.scanButton': 'Скенирај QR код',
  'relay.pasteButton': 'Налепи',
  'relay.saveAllButton': 'Додај свих {count}',
  'phrase.errChecksum': 'Ова фраза није исправна — провери да ли је нека реч погрешно укуцана или замењена.',
  'phrase.errUnknownWord': 'Реч {position} („{word}”) није међу дозвољеним речима.',
  'phrase.errWordCount': 'То је {count} речи — фраза их захтева {need}.',
  'relay.instancesTitle': 'Na ovom releju',
  'relay.noInstances': 'Ниједна инстанца ове групе тренутно није на мрежи. Провери фразу и да ли ради бар једна инстанца.',
};
