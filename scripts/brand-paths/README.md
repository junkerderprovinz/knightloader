...
Linux, Docker and Android from the `brands` set, the puzzle piece from the
`solid` set. The icons are **CC BY 4.0**, which asks for attribution and nothing
else. Copyright 2024 Fonticons, Inc.
...
Foundation, Docker or Google. The puzzle piece is no one's mark; it stands for
