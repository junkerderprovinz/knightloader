...
const TYPE_LABEL: Record<FileCategory, TranslationKey> = {
 ...
 * value with no tasks left stays at a count of 0, or nobody could switch it off.
 */
function buildFacet(
  tasks: Task[],
...
// The whole row answers a click: a <label> hands it to the switch inside.
function FacetRow({
  checked,
  label,
  count,
  ...
  return (
    <label
      className="flex w-full cursor-pointer items-center gap-2 rounded-[var(--radius-control)] px-1.5 py-1 text-xs
 ...
 * any narrowing, so a count never shrinks because another switch is on.
 */
  ...
  const hostOptions = useMemo(
    () =>
      buildFacet(tasks, selection.host, (x) => ({
        value: hostOf(x),
        label: hostOf(x) || t('collector.facets.unknownHost'),
      })),
    [tasks, selection.host, t],
  );
  const typeOptions = useMemo(
    () =>
      buildFacet(tasks, selection.fileType, (x) => {
        const cat = fileTypeOf(x);
        return { value: cat, label: t(TYPE_LABEL[cat]) };
      }),
    [tasks, selection.fileType, t],
  );
