...
export const en = {
  ...
  'instances.pairViaRelay': 'Connected through the relay - that instance has no address this one can reach directly.',
  ...
  'instances.refused': 'Reached, but it refused this instance - pair the two again with a code. An instance revokes every credential it issued whenever its password is set or changed.',
  ...
  'settings.access.network.desktopNote': 'This is the desktop build. It does not serve the API over the network at all, so there is no address here to open on another device.',
  'settings.access.network.hint': 'Open this instance\'s own interface on another device on the same network - a phone, another browser. This is not for connecting two KnightLoaders together; that is the Remote access card below.',
  ...
  'settings.access.tsnet.title': 'Connect from anywhere',
  'settings.access.tsnet.body': 'Log in once with your Tailscale account, and this instance gets its own public address - reachable from a phone, the browser extension, or another KnightLoader, with nothing to install on their side.',
  'settings.access.tsnet.hostnameLabel': 'Name',
  'settings.access.tsnet.hostnameHint': 'The left-hand part of the address this instance gets - "knightloader" becomes knightloader.<your-tailnet>.ts.net. Optional, defaults to "knightloader".',
  'settings.access.tsnet.hostnamePlaceholder': 'knightloader',
  'settings.access.tsnet.connect': 'Connect',
  'settings.access.tsnet.connecting': 'Connecting…',
  'settings.access.tsnet.disconnect': 'Disconnect',
  'settings.access.tsnet.disconnecting': 'Disconnecting…',
  'settings.access.tsnet.loginPrompt': 'Open this link to finish logging in:',
  ...
  'settings.access.phrase.stateConnected': 'Connected. Your other instances with this phrase can reach this one.',
  ...
  'settings.access.remote.stateBoth': 'Another KnightLoader can reach this one on this network, and from anywhere through your relay.',
  'settings.access.remote.stateRelay': 'Another KnightLoader can reach this one from anywhere through your relay.',
  'settings.access.remote.stateNone': 'Nothing can reach this one yet. This is the desktop build, which serves nothing over the network, so a relay is the way to connect it.',
  ...
  'settings.access.relay.body': 'A relay is a small server both instances dial out to, so two KnightLoaders that cannot reach each other directly - each behind its own router - still find each other through it. Set one relay up (self-host it, or run it on one of your own instances below), then enter that exact same address and key on every instance you want connected to it, including this one.',
  ...
  'settings.access.relay.urlHint': 'Where the relay itself answers. It does not have to be a domain - a plain IP address and port work fine as long as every connecting instance can reach it. Use https:// with a real domain once the relay needs to be reached from outside your own network; a bare IP with http:// is fine for a same-network test. Clearing it disconnects from the relay and leaves every other way of reaching this instance untouched.',
  'settings.access.relay.bothSidesHint': 'Enter the same address and key here that you enter on this same page of every other instance you want to connect - a relay only introduces instances that were both told about it.',
  'settings.access.relay.keyLabel': 'Relay key',
  'settings.access.relay.keyHint': 'One shared secret is the whole of the authorisation, so every instance that should see the others gets the same one. It is stored encrypted here, like a debrid key, and is never shown again once saved - if it is lost, make a new one and enter that same key everywhere, on every instance.',
