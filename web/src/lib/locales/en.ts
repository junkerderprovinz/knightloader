...
export const en = {
  ...
  'settings.access.relay.body': 'The third point two instances behind different routers can both reach. This project\'s by default, and replaceable.',
  'settings.access.relay.seesButton': 'What can it see?',
  'settings.access.relay.seesTip': 'What it cannot see: the path being called, the body, your task list, the file bytes, and the app\'s own credential. All of it is sealed with a key derived from your twelve words under a different domain than the routing key, so the relay holds one and cannot work out the other. This instance\'s name is in there too now, which used to fall back to the machine\'s hostname and reached the relay in the clear.\n\nWhat it can see: the routing key your group shares, the public address each instance dials in from, an instance id, and when each one arrives, leaves and sends something, including how big it was.\n\nThat is the outside of the envelope, and no amount of encryption removes it. If it matters to you, run your own relay - the card below is how.',
  'settings.access.relay.own': 'Your own',
  'settings.access.relay.project': 'The project\'s',
  'settings.access.relay.whichHint': 'Which relay this instance is dialling right now, read from the running client rather than from a saved setting.',
  'settings.access.relay.leadProject': 'Your instances meet on the relay this project runs. Everything they say to each other is sealed with a key derived from your twelve words, which the relay is never given and cannot work out from what it is given.',
  'settings.access.relay.leadOwn': 'Your instances meet on a relay you run yourself. Nothing about this instance reaches anybody else\'s machine.',
  'settings.access.relay.address': 'Address',
  'settings.access.ownRelay.title': 'Your own relay',
  'settings.access.ownRelay.body': 'Three ways off the project\'s relay, from the least work to the most.',
  'settings.access.ownRelay.lead': 'A relay is a third point both instances dial out to, so neither has to be reachable from the other. It does have to be reachable from both, and that is the only thing deciding where it can live.',
  'settings.access.ownRelay.serveLabel': 'This instance is the relay',
  'settings.access.ownRelay.serveHint': 'Runs the relay under /relay/connect on the address this instance already answers on, behind the same reverse proxy and the same certificate. No second container, nothing extra to install. It only helps if this instance is reachable from outside: a relay nobody can dial is not a relay.',
  'settings.access.ownRelay.serveClients': '{count} connected right now.',
  'settings.access.ownRelay.addressLabel': 'Relay address',
  'settings.access.ownRelay.addressPlaceholder': 'e.g. wss://relay.example.com/relay/connect',
  'settings.access.ownRelay.save': 'Save',
  'settings.access.ownRelay.saved': 'Relay address saved',
  'settings.access.ownRelay.reset': 'Back to the project\'s relay',
  'settings.access.ownRelay.everyInstance': 'This address is stored per instance and does not travel with your twelve words. Enter it on every instance in the group, or the ones you miss will go on meeting somewhere else.',
  'settings.access.ownRelay.containerLabel': 'As its own container',
  'settings.access.ownRelay.containerHint': 'For the case neither way above can serve: no machine of yours is reachable from outside, so the relay has to live somewhere that is - a small VPS, for instance.\n\nThe image keeps no state beyond the list of who is connected right now, declares no volume, and needs no configuration but the port. There is nothing here to back up.\n\nThere is deliberately no install button: the machine that needs a relay is, by definition, not the machine that can host one for it.',
  ...
  'settings.access.phrase.howWhat': 'The words are a secret, not an address. Each instance turns them into the same two keys: one to find the others, one to encrypt with.\n\nThe first key is how they meet. Every instance dials out to the same meeting point on the internet and presents it, and whoever presents the same one is introduced. Because both sides dial out, neither has to be reachable from outside - no port forwarding, no domain, nothing to open up. The meeting point only ever receives a scrambled form of that key, never your words, so whoever runs it cannot work out your phrase or let themselves into your group.\n\nThe second key is why the meeting point cannot read what it carries. Everything sent through it - every request, every answer, every access token - is encrypted with a key it never receives and cannot compute from the one it has. It sees which instance a message is for, because it has to route it, and nothing else.',
  'settings.access.phrase.statusConnected': 'Connected',
  'settings.access.phrase.statusDisconnected': 'Not connected',
  'settings.access.phrase.statusHint': 'Your other instances with this phrase reach this one.',
