...
export const en = {
  ...
  'settings.access.relay.urlHint': 'The address your own relay answers on, behind TLS for anything beyond a LAN test. Clearing it disconnects from the relay and leaves every other way of reaching this instance untouched.',
  'settings.access.relay.keyLabel': 'Relay key',
  'settings.access.relay.keyHint': 'One shared secret is the whole of the authorisation, so every instance that should see the others gets the same one. It is stored encrypted here, like a debrid key, and is never shown again once saved - if it is lost, make a new one and enter that everywhere.',
