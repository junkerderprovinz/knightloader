...
export const en = {
  ...
  'settings.downloads.autoConfirmOff': '"Start added links immediately" is switched off under General, so no countdown runs here.',
  ...
  'accounts.routing.priorityHint': 'Which service is asked first when more than one can fetch the same link. Drag to rearrange.\n\nThe list holds what you choose between: debrid accounts such as TorBox and AllDebrid, your own hoster accounts, torrents and remote storage. The order only decides between the ones that can actually take a link.\n\nJDownloader, yt-dlp and the plain direct download stand below it and are not ordered, because the right one depends on the link. KnightLoader sends a filehoster link to JDownloader, a video site to yt-dlp and an ordinary file to the direct download on its own, after everything in the list.',
  ...
  'accounts.routing.resolver.hostheaders': 'Header profiles',
  'accounts.routing.resolver.remotefs': 'FTP, SFTP and WebDAV',
  'accounts.routing.loginRow': 'Hoster account, through JDownloader',
  'accounts.routing.automaticTitle': 'Automatic, per link',
  ...
  'settings.schedule.orderHint':
    'Rows are applied in order, top to bottom, and where two windows overlap, the later row wins. So a broad "pause every night" above a narrow exception leaves the exception in force, while the same two rows the other way round do not.',
  ...
  'settings.schedule.endHint':
    'An end time before the start time means the window runs past midnight and ends the following morning. An end time equal to the start time is refused, because it could mean a whole day or no time at all.',
