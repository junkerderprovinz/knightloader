...
2. **Depth**: full, with peer/seed UI, port/NAT guidance and tracker awareness.
...
5. **Private trackers**: DHT and Peer Exchange are automatically disabled
   ...
   **Status: delivered**, through a different door than this section
   ...
   github.com/anacrolix/torrent@eda2204d2568…`), with no gopeed change and
   no fork.
...
Reconfirmed, unchanged: the original guardrail (no
...
- `Resolve` hands the magnet URI or `.torrent` bytes to the embedded
  ...
  second, parallel download pipeline. The fact above means there is nothing
  new to wire at the engine level, only a new call shape into the same
  `download.Downloader`.
- `.torrent` file upload is a new intake path (see UI below), not a URL at
  all, because the collector's existing add-link flow assumes a string; this
...
description) where the existing reconnect actions take none, so `soap()`'s
...
- **New settings page**, "Torrents", on its own tab (own icon; check
...
- **`SeedRatioTarget` vs `SeedDurationSeconds`**: confirm "whichever is
  ...
  controller logic for it. It was not settled when the decisions were taken.
