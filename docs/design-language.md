...
`web/src/index.css` and `web/src/lib/appearance.ts` remain here as KnightLoader's own live implementation of the language, not the spec itself. When a rule changes upstream, port it here the same way any other adopting app would (see the GlimStone repo's "Adopting GlimStone in another app" section).
