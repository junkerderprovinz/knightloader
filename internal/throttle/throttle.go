...
//
// Nothing calls this yet. The app has two neighbouring controls and this is
// neither: the queue halt leaves running downloads running (a queue switch,
// see App.SetHalted) and the hard stop tears them down. This is the third,
// JDownloader's pause mode: bytes stop, connections stay, so the line is free
// without losing the transfers, and it still wants a button and a label.
//
// The paused reader unblocks on a channel rather than by polling, and the
// tokens that accrued during the pause are spent, so a resume cannot pay out
// an hour as one burst. Wiring the button up is one SetPaused call beside
// whatever writes the mode.
