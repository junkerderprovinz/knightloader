...
// of them.
//
// Go decides time.Local once, on first use, from the environment the process
// started in, so t.Setenv("TZ", …) inside a test changes nothing, and a test
// written against the environment would pass on a runner that happens to be on
// UTC and fail on a machine in Europe/Berlin, or pass on both while checking
// nothing. With the facts injected, every one of the four verdicts below is
...
type clockFacts struct {
	...
	ZoneName string
	Offset   int
	// LocalName is time.Local.String(), and no verdict is raised on it. Go's
	// initLocal overwrites the name whenever the zone came from
	// /etc/localtime rather than from TZ, so a container with /etc/localtime
	// bind-mounted, an ordinary setup, reports the literal string "Local" and
	// looks broken to anything that reads it as a zone name. It travels as
	// supporting text.
	LocalName string
...
//
// The finding is that Go falls back to UTC in silence. TZ=Europe/Berln, a
// typo, and TZ unset are indistinguishable from inside the process except that
// local time is UTC in both, and no error is raised anywhere. The first
// symptom is a nightly window that ran an hour or two off, weeks later.
func Clock() Check {
	f := observeClock()
	return clockCheck(f)
}
...
func observeClock() clockFacts {
	...
	// test does, so the database is the container image's tzdata package. A
	// rebuild onto scratch or distroless removes every DST transition in the
...
func clockCheck(f clockFacts) Check {
	...
	switch {
	case !f.ZoneDBOK:
		// Checked first, and the order is the difference between a useful row
		// and a misleading one. With no zone database even a correctly
		// spelled TZ fails to load, so testing the spelling first would
		// report "check your spelling" at somebody whose image simply has no
		// zone data in it.
	...
	case f.TZ != "" && !f.TZLoadOK && f.Offset == 0 && f.ZoneName == "UTC":
		...
		// is one fact, that the process is running on UTC anyway is the
		// other, and only the pair proves the silent fallback happened. On
		// Windows the pair never occurs, because Go ignores TZ there and
		// takes the zone from the system, so a desktop install with a stray
		// TZ is not accused of it.
		c.Verdict = VerdictFail
		c.Code = CodeUTCFallback

	case f.TZ == "" && f.Offset == 0 && f.ZoneName == "UTC":
		// Warn and not fail: running on UTC is a working configuration, and
		// on a container with no TZ set it is the expected one. It is here
		// because a window written as 23:00 then means 23:00 UTC, not 23:00
		// where the person who typed it lives.
		c.Verdict = VerdictWarn
		c.Code = CodeTZUnset
