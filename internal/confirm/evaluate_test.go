...
// TestOnlyOfflineIsEverExcludable checks that Evaluate takes Offline=false at
// its word. Keeping unknown availability from becoming Offline=true is the
// app layer's job (internal/app/app_confirm_test.go).
...
// TestEvaluateReasonsOnlyBlameTheDecidingFact covers an item that is both a
// duplicate and offline where only the offline policy holds it back.
